...
class PWM(object):
    ...
    RPI_REVISION_1_MODULE_B = ["Beta", "0002", "0003",
            ...
            for line in f:
                if line.startswith('Revision'):
                    if line[11:-1] in self.RPI_REVISION_0:
                        ...
                        print(
                            "Error. Pi revision didn't recognize, "
                            "module number: %s" % line[11:-1])
                        print('Exiting...')
                        quit()
    ...
    def _check_i2c(self):
        ...
        if '/dev/i2c-%d' % bus_number in output.split(' '):
            ...
            print("Seems like I2C has not been set. "
                  "Use 'sudo raspi-config' to set I2C")
        ...
        if "%02X" % self.address in addresses:
            print("Wierd, I2C device is connected. Try to run the program "
                  "again. If the problem's still, email the error message to "
                  "service@sunfounder.com")
        else:
            print("Device is missing.")
            print("Check the address or wiring of PCA9685 servo driver, or "
                  "email the error message to service@sunfounder.com")
            print('Exiting...')
        quit()
    ...
    def write(self, channel, on, off):
        ...
        if self._DEBUG:
            print((self._DEBUG_INFO, 'Set channel "%d" to value "%d"' %
                   (channel, off)))
    ...
    def debug(self, debug):
        ...
        if debug in (True, False):
            ...
            raise ValueError(
                "debug must be 'True' (Set debug on) or 'False' (Set debug "
                "off), not '{0}'".format(debug))
